...
void Projectile::update(float dt) {
  m_position.x = m_position.x + (m_direction.x * m_speed * dt);
  m_position.y = m_position.y + (m_direction.y * m_speed * dt);
}

void Projectile::draw(Renderer &renderer) const {

  renderer.drawCircle(m_position.x, m_position.y, 2, COLOR_GREEN);
}
